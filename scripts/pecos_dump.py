...
def recover_enrollment_parquet_from_raw() -> Path | None:
    ...
    logger.info("Recovering enrollment parquet from existing raw CSV: %s", raw_csv)
    df = save_parquet_from_csv(raw_csv, parquet_path, logger=logger)
    warn_if_schema_drift(df, raw_csv.stem, logger)
    logger.info(
        "Recovered enrollment parquet: %s (%s rows, %s columns)",
        parquet_path,
        f"{len(df):,}",
        len(df.columns),
    )
...
def main():
    ...
    if enrollment_parquet is None or not enrollment_parquet.exists():
        ...
        if enrollment_parquet is None or not enrollment_parquet.exists():
            logger.error("Could not find an enrollment parquet or recover one from raw CSV.")
            return
